...
class CarouselSlider extends HTMLElement {
  ...
  init() {
    ...
    // this.nav => this.navs
    // this.prevBtn => this.prevBtns
    // this.nextBtn => this.nextBtns

    if (this.navs.length) {
      this.prevBtns = this.querySelectorAll('button[name="prev"]');
      this.nextBtns = this.querySelectorAll('button[name="next"]');
    }
  ...
  initSlider() {
    ...
    this.addListeners();
    if (this.totalPages < 2 || !this.navs.length) return;
  ...
  addListeners() {
    ...
    if (this.navs.length) {
      ...
      this.navs.forEach(nav => nav.addEventListener('click', this.navClickHandler));
    }
  ...
  removeListeners() {
    ...
    this.navs.forEach(nav => nav.removeEventListener('click', this.navClickHandler));
   ...
   */
  handleResize() {
    if (this.navs.length) this.removeListeners();
    this.initSlider();
  }
   ...
   */
  setButtonStates() {
    this.prevBtns.forEach(prevBtn => prevBtn.disabled = this.getSlideVisibility(this.slides[0]) && this.slider.scrollLeft === 0);
    this.nextBtns.forEach(nextBtn => nextBtn.disabled = this.getSlideVisibility(this.slides[this.slides.length - 1]));
  }
}
